...
namespace NSoft.NFramework.FusionCharts.WebHost.Powers.SplineCharts {
    ...
    public class MultiSeriesDataProvider : NSoft.NFramework.FusionCharts.Web.DataXmlHandlerBase {
        ...
        private static void FillData(DataSetElement dataSet, int factoryId) {
            ...
            foreach(var output in outputs) {
                ...
                if((num++ % 2) == 0)
                    ...
                    set.Link.SetLink(FusionLinkMethod.PopUp, WebTool.GetScriptPath("~/Charts/Ajax/Default.aspx?FactoryId=") + factoryId);
                    set.Link.Width = 600;
                    set.Link.Height = 400;
                }

                dataSet.SetElements.Add(set);
            }
        }
