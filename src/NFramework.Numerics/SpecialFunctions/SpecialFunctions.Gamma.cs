...
namespace NSoft.NFramework.Numerics {
    public static partial class SpecialFunctions {
        /// <summary>
        /// The order of the <see cref="GammaLn(double)"/> approximation.
        /// </summary>
        private const int Gamma_N = 10;

        /// <summary>
        /// Auxiliary variable when evaluating the <see cref="GammaLn(double)"/> function.
        /// </summary>
        private const double Gamma_R = 10.900511;

        /// <summary>
        /// Polynomial coefficients for the <see cref="GammaLn(double)"/> approximation.
        /// </summary>
        private static readonly double[] Gamma_dk = new[]
                                                    {
        ...
        /// </remarks>
        public static double GammaLn(this double z) {
            if(z < 0.5) {
                double s = Gamma_dk[0];
                ...
                return MathTool.LnPI
                       - Math.Log(Math.Sin(MathTool.Pi * z))
                       - Math.Log(s)
                       - MathTool.Ln2SqrtEOverPi
                       - ((0.5 - z) * Math.Log((0.5 - z + Gamma_R) / MathTool.E));
            }
            else {
                double s = Gamma_dk[0];
                for(int i = 1; i <= Gamma_N; i++) {
                ...
                return Math.Log(s)
                       + MathTool.Ln2SqrtEOverPi
                       + ((z - 0.5) * Math.Log((z - 0.5 + Gamma_R) / MathTool.E));
            }
        }
        ...
        public static double Gamma(this double z) {
            if(z < 0.5) {
                ...
                return MathTool.Pi / (Math.Sin(MathTool.Pi * z)
                                      * s
                                      * MathTool.TwoSqrtEOverPi
                                      * Math.Pow((0.5 - z + Gamma_R) / MathTool.E, 0.5 - z));
            }
            else {
                double s = Gamma_dk[0];
                for(int i = 1; i <= Gamma_N; i++) {
                ...
                return s * MathTool.TwoSqrtEOverPi * Math.Pow((z - 0.5 + Gamma_R) / MathTool.E, z - 0.5);
            }
        }
        ...
        public static double GammaUpperRegularized(double a, double x) {
            ...
            do {
                ...
                if(qk.ApproximateEqual(0.0) == false) {
                    r = pk / qk;
                    t = Math.Abs((ans - r) / r);
                    ans = r;
                }
        ...
        /// <returns></returns>
        public static IEnumerable<double> GammaLn(this IEnumerable<double> source) {
            source.ShouldNotBeNull("source");
            return source.Select(x => GammaLn(x));
        }
        ...
        /// <returns></returns>
        public static IEnumerable<double> GammaLn<T>(this IEnumerable<T> source, Func<T, double> selector) {
            return GammaLn(source.Select(selector));
        }
        ...
        /// <param name="source"></param>
        /// <returns></returns>
        public static IEnumerable<double> Gamma(this IEnumerable<double> source) {
            return source.Select(x => Gamma(x));
        }
        ...
        /// <returns></returns>
        public static IEnumerable<double> Gamma<T>(this IEnumerable<T> source, Func<T, double> selector) {
            return Gamma(source.Select(selector));
        }
    }
}