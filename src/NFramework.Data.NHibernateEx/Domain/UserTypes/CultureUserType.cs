...
namespace NSoft.NFramework.Data.NHibernateEx.Domain.UserTypes {
    /// <summary>
    /// NHibernate가 지원하는 <see cref="CultureInfo"/>에 대한 저장 공간의 기본 크기가 5로 제한되어 있다. 
    /// 중화권의 경우 5자가 넘는 경우가 있어서 128자 이상으로 저장공간을 늘렸습니다.
    /// </summary>
    public class CultureUserType : IUserType {
        /// <summary>
        ...
        public int GetHashCode(object x) {
            return (x == null) ? typeof(CultureUserType).GetHashCode() : x.GetHashCode();
        }
        ...
        public object NullSafeGet(IDataReader rs, string[] names, object owner) {
            var cultureName = NHibernateUtil.String.NullSafeGet(rs, names[0]).AsText();

            return cultureName.IsWhiteSpace()
        ...
        public void NullSafeSet(IDbCommand cmd, object value, int index) {
            var isCulture = (value != null && value is CultureInfo);

            if(isCulture)
                NHibernateUtil.String.NullSafeSet(cmd, ((CultureInfo)value).Name, index);
            else
        ...
        public SqlType[] SqlTypes {
            get { return new SqlType[] { new StringSqlType(128) }; }
        }
        ...
        public bool IsMutable {
            get { return NHibernateUtil.CultureInfo.IsMutable; }
        }
    }
}