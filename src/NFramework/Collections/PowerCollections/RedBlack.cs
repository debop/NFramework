...
namespace NSoft.NFramework.Collections.PowerCollections {
    ...
    public class RedBlackTree<T> : IEnumerable<T> {
        ...
        private readonly IComparer<T> _comparer; // interface for comparing elements, only Compare is used.
        ...
        public RedBlackTree<T> Clone() {
            var newTree = new RedBlackTree<T>(_comparer) { _count = _count };
        ...
        public bool Find(T key, bool findFirst, bool replace, out T item) {
            ...
            while(current != null) {
                ...
                if(compare < 0) {
                    ...
                    Guard.Assert(compare == 0, "compare should be zero. compare=[{0}]", compare);
                    found = current;

                    current = findFirst ? current.left : current.right;
                }
            }
            ...
            if(found != null) {
                ...
                if(IsDebugEnabled)
                    log.Debug("검색에 성공했습니다. key=[{0}], item=[{1}]", key, item);

                return true;
            }

            if(IsDebugEnabled)
                log.Debug("검색에 실패했습니다. 결과는 기본값을 설정합니다. key=[{0}]", key);
        ...
        public int FindIndex(T key, bool findFirst) {
            ...
            return findFirst
                       ? FirstItemInRange(EqualRangeTester(key), out dummy)
                       : LastItemInRange(EqualRangeTester(key), out dummy);
        }
        ...
        public T GetItemByIndex(int index) {
            ...
            var current = _root; // current search location in the tree
        ...
        public bool Insert(T item, DuplicatePolicy dupPolicy, out T previous) {
            if(IsDebugEnabled)
                log.Debug("요소[{0}] 를 추가합니다... DuplicatePolicy=[{1}]", item, dupPolicy);

            var node = _root;

            Node parent = null; // parent, grand, a great-grantparent of node.
            Node gparent = null; // parent, grand, a great-grantparent of node.
            Node ggparent = null; // parent, grand, a great-grantparent of node.
            ...
            var needStack = !((dupPolicy == DuplicatePolicy.InsertFirst) || (dupPolicy == DuplicatePolicy.InsertLast));
            Node[] nodeStack = null;
            var nodeStackPtr = 0; // first free item on the stack.
            ...
            while(node != null) {
                ...
                if(compare == 0) {
                    ...
                    if(dupPolicy == DuplicatePolicy.InsertFirst || dupPolicy == DuplicatePolicy.ReplaceFirst) {
                        ...
                        Guard.Assert(dupPolicy == DuplicatePolicy.InsertLast || dupPolicy == DuplicatePolicy.ReplaceLast);
                ...
                Guard.Assert(compare != 0, "compare 값이 0이면 안됩니다. 중복 데이타를 처리하지 못했습니다.");
            ...
            if(wentLeft)
                ...
                Guard.Assert(_root == null, "_root 가 null이어야 새로운 노드를 설정합니다...");
                _root = node;
            }
        ...
        private Node InsertSplit(Node ggparent, Node gparent, Node parent, Node node, out bool rotated) {
            ...
            if(parent != null && parent.IsRed) {
                ...
                gparent.ShouldNotBeNull("gparent");
        ...
        private void Rotate(Node node, Node child, Node gchild) {
            if(gchild == child.left) {
                ...
                Guard.Assert(gchild == child.right);
        ...
        public RangeTester BoundedRangeTester(bool useFirst, T first, bool useLast, T last) {
            return item => {
                       ...
                       if(useLast && _comparer.Compare(last, item) <= 0)
        ...
        public RangeTester LowerBoundedRangeTester(T first, bool inclusive) {
            return item => inclusive
                               ? (_comparer.Compare(first, item) > 0 ? -1 : 0)
                               : (_comparer.Compare(first, item) >= 0 ? -1 : 0);
        }
        ...
        public RangeTester UpperBoundedRangeTester(T last, bool inclusive) {
            return item => {
                       if(inclusive) {
                           return _comparer.Compare(last, item) < 0 ? 1 : 0;
                       }
                       return _comparer.Compare(last, item) <= 0 ? 1 : 0;
                   };
        }
        ...
        public RangeTester EqualRangeTester(T equalTo) {
            return item => _comparer.Compare(item, equalTo);
        }
        ...
        private IEnumerable<T> EnumerateRangeInOrder(RangeTester rangeTester, Node node) {
            ...
            if(node != null) {
                var compare = rangeTester(node.item);
        ...
        public bool DeleteItemFromRange(RangeTester rangeTester, bool deleteFirst, out T item) {
            ...
            for(;;) {
                Guard.Assert(parent == null || parent.IsRed);
                Guard.Assert(sib == null || !sib.IsRed);
                Guard.Assert(!node.IsRed);

                if((node.left == null || !node.left.IsRed) && (node.right == null || !node.right.IsRed)) {
                    // node has two black children (null children are considered black).
                    if(parent == null) {
                        // Special case for the root.
                        Guard.Assert(node == _root);
                        node.IsRed = true;
                    }
            ...
            if(node.left != null) {
                ...
                Guard.Assert(!node.IsRed && replacement.IsRed);
            ...
            if(parent == null) {
                Guard.Assert(_root == node);
                _root = replacement;
            }
            else if(parent.left == node)
                parent.left = replacement;
            else {
                Guard.Assert(parent.right == node);
        ...
        private static int CountRangeUnderNode(RangeTester rangeTester, Node node, bool belowRangeTop, bool aboveRangeBottom) {
            if(node == null)
                return 0;
        ...
        private void PrintSubTree(Node node, string prefixNode, string prefixChildren) {
            ...
            //
            if(IsDebugEnabled)
                log.Debug("{0}{1} {2,4} {3}", prefixNode, node.IsRed ? "@@" : "..", node.Count, node.item.ToString());
        ...
        public void Validate() {
            _comparer.ShouldNotBeNull("_comparer");

            if(_root == null) {
                Guard.Assert(_count == 0, "Count in empty tree should be 0.");
            }
            else {
                Guard.Assert(_root.IsRed == false, "Root is not black");

                int blackHeight;
                int nodeCount = ValidateSubTree(_root, out blackHeight);

                Guard.Assert(nodeCount == _count, "Node count of tree is not correct.");
            }
        }
        ...
        private int ValidateSubTree(Node node, out int blackHeight) {
            ...
            if(node.left != null)
                Guard.Assert(_comparer.Compare(node.left.item, node.item) <= 0, "Left child is not less than or equal to node");

            if(node.right != null)
                Guard.Assert(_comparer.Compare(node.right.item, node.item) >= 0, "Right child is not greater than or equal to node");

            // Check that the two-red rule is not violated.
            if(node.IsRed) {
                if(node.left != null)
                    Guard.Assert(node.left.IsRed == false, "Node and left child both red");

                if(node.right != null)
                    Guard.Assert(node.right.IsRed == false, "Node and right child both red");
            }
            ...
            var leftCount = ValidateSubTree(node.left, out leftBlackHeight);
            var rightCount = ValidateSubTree(node.right, out rightBlackHeight);
            var ourCount = leftCount + rightCount + 1;

            Guard.Assert(ourCount == node.Count);

            // Validate the equal black-height rule.
            Guard.Assert(leftBlackHeight == rightBlackHeight, "Black heights are not equal");
