...
namespace NSoft.NFramework.Collections.PowerCollections {
    ...
    public class OrderedMultiDictionary<TKey, TValue> : MultiDictionaryBase<TKey, TValue> {
        ...
        private RedBlackTree<KeyValuePair<TKey, TValue>>.RangeTester KeyRange(TKey key) {
            return pair => _keyComparer.Compare(pair.Key, key);
        }
        ...
        private RedBlackTree<KeyValuePair<TKey, TValue>>.RangeTester DoubleBoundedKeyRangeTester(TKey first, bool firstInclusive,
                                                                                                 TKey last, bool lastInclusive) {
            return pair => {
                       if(firstInclusive) {
                           if(_keyComparer.Compare(first, pair.Key) > 0)
                       ...
                       if(lastInclusive) {
                           if(_keyComparer.Compare(last, pair.Key) < 0)
        ...
        private RedBlackTree<KeyValuePair<TKey, TValue>>.RangeTester LowerBoundedKeyRangeTester(TKey first, bool inclusive) {
            return pair => inclusive
                               ? (_keyComparer.Compare(first, pair.Key) > 0 ? -1 : 0)
                               : (_keyComparer.Compare(first, pair.Key) >= 0 ? -1 : 0);
        }
        ...
        private RedBlackTree<KeyValuePair<TKey, TValue>>.RangeTester UpperBoundedKeyRangeTester(TKey last, bool inclusive) {
            return pair => inclusive
                               ? (_keyComparer.Compare(last, pair.Key) < 0 ? 1 : 0)
                               : (_keyComparer.Compare(last, pair.Key) <= 0 ? 1 : 0);
        }
        ...
        private OrderedMultiDictionary(bool allowDuplicateValues, int keyCount, IComparer<TKey> keyComparer,
                                       IComparer<TValue> valueComparer, IComparer<KeyValuePair<TKey, TValue>> comparer,
        ...
        public override sealed void Add(TKey key, TValue value) {
            ...
            _tree.Insert(pair, _allowDuplicateValues ? DuplicatePolicy.InsertLast : DuplicatePolicy.ReplaceLast, out dummy);
        }
        ...
        public override sealed bool Remove(TKey key, TValue value) {
            KeyValuePair<TKey, TValue> dummy;
            bool found = _tree.Delete(NewPair(key, value), false, out dummy);

            if(found && !ContainsKey(key))
                --_keyCount; // Removed the last value associated with the key.
        ...
        public override sealed bool Remove(TKey key) {
            if(_tree.DeleteRange(KeyRange(key)) > 0) {
                --_keyCount;
                return true;
            }
            return false;
        ...
        protected override sealed bool EqualValues(TValue value1, TValue value2) {
            return _valueComparer.Compare(value1, value2) == 0;
        }
        ...
        public override sealed bool Contains(TKey key, TValue value) {
            KeyValuePair<TKey, TValue> dummy;
            return _tree.Find(NewPair(key, value), true, false, out dummy);
        }
        ...
        public override sealed bool ContainsKey(TKey key) {
            KeyValuePair<TKey, TValue> dummy;
            return (_tree.FirstItemInRange(KeyRange(key), out dummy) >= 0);
        }
        ...
        private IEnumerator<TValue> EnumerateValuesForKey(TKey key) {
            return _tree.EnumerateRange(KeyRange(key)).Select(pair => pair.Value).GetEnumerator();
        }
        ...
        protected override sealed IEnumerator<TKey> EnumerateKeys() {
            return EnumerateKeys(_tree.EntireRangeTester, false);
        }
        ...
        protected override sealed int CountValues(TKey key) {
            return _tree.CountRange(KeyRange(key));
        }
        ...
        public OrderedMultiDictionary<TKey, TValue> Clone() {
            return new OrderedMultiDictionary<TKey, TValue>(_allowDuplicateValues, _keyCount, _keyComparer, _valueComparer, _comparer,
                                                            _tree.Clone());
        }
        ...
        private sealed class KeyValuePairsCollection : ReadOnlyCollectionBase<KeyValuePair<TKey, TValue>> {
            private readonly OrderedMultiDictionary<TKey, TValue> _myDictionary;
            ...
            public override IEnumerator<KeyValuePair<TKey, TValue>> GetEnumerator() {
                return _myDictionary._tree.GetEnumerator();
            }

            public override bool Contains(KeyValuePair<TKey, TValue> pair) {
                KeyValuePair<TKey, TValue> dummy;
                return _myDictionary._tree.Find(pair, true, false, out dummy);
            }
        }
        ...
        public class View : MultiDictionaryBase<TKey, TValue> {
            ...
            private readonly RedBlackTree<KeyValuePair<TKey, TValue>>.RangeTester _rangeTester;
                                                                                  // range tester for the range being used.
            ...
            private bool KeyInView(TKey key) {
                return _rangeTester(NewPair(key, default(TValue))) == 0;
            }
            ...
            protected override sealed IEnumerator<TKey> EnumerateKeys() {
                return _myDictionary.EnumerateKeys(_rangeTester, _reversed);
            }
            ...
            protected override sealed bool TryEnumerateValuesForKey(TKey key, out IEnumerator<TValue> values) {
                ...
                return _myDictionary.TryEnumerateValuesForKey(key, out values);
            }
            ...
            public override sealed int Count {
                get {
                    ...
                    using(var enumKeys = _myDictionary.EnumerateKeys(_rangeTester, _reversed)) {
            ...
            public override sealed bool Contains(TKey key, TValue value) {
                ...
                return _myDictionary.Contains(key, value);
            }
            ...
            protected override sealed int CountValues(TKey key) {
                ...
                return _myDictionary.CountValues(key);
            }
            ...
            public override sealed bool Remove(TKey key, TValue value) {
                ...
                return _myDictionary.Remove(key, value);
            }
