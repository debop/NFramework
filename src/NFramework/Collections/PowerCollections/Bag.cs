...
namespace NSoft.NFramework.Collections.PowerCollections {
    ...
    public class Bag<T> : CollectionBase<T> {
        ...
        public Bag<T> Clone() {
            return new Bag<T>(_equalityComparer, _keyEqualityComparer, _hash.Clone(null), _count);
        }
        ...
        public Bag<T> CloneContents() {
            ...
            var newHash = new Hash<KeyValuePair<T, int>>(_equalityComparer);

            // Clone each item, and add it to the new ordered bag.
            foreach(KeyValuePair<T, int> pair in _hash) {
                KeyValuePair<T, int> dummy;

                var newKey = ((itemIsValueType == false) && (pair.Key != null))
                                 ? (T)pair.Key //(T) (((ICloneable) pair.Key).Clone())
                                 : pair.Key;
        ...
        public IEnumerable<T> DistinctItems() {
            return _hash.Select(pair => pair.Key);
        }
        ...
        public override sealed void Add(T item) {
            ...
            KeyValuePair<T, int> existing;

            if(_hash.Insert(pair, false, out existing) == false) {
                // The item already existed, so update the count instead.
                KeyValuePair<T, int> newPair = NewPair(existing.Key, existing.Value + 1);
        ...
        public void AddRepresentative(T item) {
            ...
            if(_hash.Insert(pair, false, out existing) == false) {
                // The item already existed, so update the count instead.
                KeyValuePair<T, int> newPair = NewPair(pair.Key, existing.Value + 1);
                _hash.Insert(newPair, true, out pair);
            }
            ++_count;
        }
        ...
        public override sealed bool Remove(T item) {
            ...
            if(_hash.Delete(NewPair(item), out removed)) {
                if(removed.Value > 1) {
                    ...
                    var newPair = NewPair(removed.Key, removed.Value - 1);
                    KeyValuePair<T, int> dummy;
        ...
        public bool IsProperSupersetOf(Bag<T> otherBag) {
            ...
            return otherBag.Count < Count && IsSupersetOf(otherBag);
        }
        ...
        public bool IsDisjointFrom(Bag<T> otherBag) {
            ...
            return smaller.DistinctItems().All(item => larger.Contains(item) == false);
        }
