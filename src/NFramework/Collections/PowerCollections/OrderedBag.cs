...
namespace NSoft.NFramework.Collections.PowerCollections {
    ...
    public class OrderedBag<T> : CollectionBase<T> {
        ...
        public OrderedBag() : this(Comparers.DefaultComparer<T>()) {}
        ...
        public OrderedBag(Comparison<T> comparison) : this(Comparers.ComparerFromComparison<T>(comparison)) {}
        ...
        /// <exception cref="InvalidOperationException">T does not implement IComparable&lt;TKey&gt;.</exception>
        public OrderedBag(IEnumerable<T> collection) : this(collection, Comparers.DefaultComparer<T>()) {}
        ...
        public OrderedBag<T> Clone() {
            return new OrderedBag<T>(_comparer, tree.Clone());
        }
        ...
        public OrderedBag<T> CloneContents() {
            ...
            var clone = new OrderedBag<T>(_comparer);
            ...
            foreach(T item in this) {
                ...
                if(itemIsValueType)
                    ...
                    itemClone = Equals(item, null) ? default(T) : item; //(T) (((ICloneable) item).Clone());
                }
        ...
        public int RemoveMany(IEnumerable<T> collection) {
            ...
            if(collection == this) {
                ...
                count += collection.Count(item => Remove(item));
            }
        ...
        public override sealed void Clear() {
            ...
            tree = new RedBlackTree<T>(_comparer);
        }
        ...
        private void CheckEmpty() {
            Guard.Assert(Count > 0, Strings.CollectionIsEmpty);
        }
        ...
        private void CheckConsistentComparison(OrderedBag<T> otherBag) {
            ...
            if(Equals(_comparer, otherBag._comparer) == false)
        ...
        public bool IsDisjointFrom(OrderedBag<T> otherBag) {
            ...
            return smaller.All(item => !larger.Contains(item));
        }
        ...
        public void IntersectionWith(OrderedBag<T> otherBag) {
            ...
            var newTree = new RedBlackTree<T>(_comparer);
        ...
        public OrderedBag<T> Intersection(OrderedBag<T> otherBag) {
            ...
            // Enumerate each of the items in the smaller bag. Add items that need to be
            // added to the intersection.
            var result = new OrderedBag<T>(_comparer);
            foreach(T item in smaller) {
                if(atBeginning || _comparer.Compare(item, previous) != 0) {
                    copiesInLarger = larger.NumberOfCopies(item);
                    copiesInSmaller = 1;
        ...
        public OrderedBag<T> SymmetricDifference(OrderedBag<T> otherBag) {
            ...
            var enum2 = otherBag.GetEnumerator();

            var valid1 = enum1.MoveNext();
            var valid2 = enum2.MoveNext();
        ...
        private class ListView : ReadOnlyListBase<T> {
            ...
            public override sealed int Count {
                get {
                    ...
                    return _myBag.tree.CountRange(_rangeTester);
                }
            }
            ...
            public override sealed T this[int index] {
                get {
                    if(_entireTree) {
                        return _reversed ? _myBag[_myBag.Count - 1 - index] : _myBag[index];
                    }

                    T dummy;
                    var firstIndex = _myBag.tree.FirstItemInRange(_rangeTester, out dummy);
                    var lastIndex = _myBag.tree.LastItemInRange(_rangeTester, out dummy);

                    if(firstIndex < 0 || lastIndex < 0 || index < 0 || index >= (lastIndex - firstIndex + 1))
                        throw new ArgumentOutOfRangeException("index");

                    return _reversed ? _myBag[lastIndex - index] : _myBag[firstIndex + index];
                }
            }

            public override sealed int IndexOf(T item) {
                if(_entireTree) {
                    return _reversed ? _myBag.Count - 1 - _myBag.LastIndexOf(item) : _myBag.IndexOf(item);
                }
                ...
                if(_reversed) {
                    var indexInSet = _myBag.tree.FindIndex(item, false);
                    if(indexInSet < 0)
                        return -1;

                    var indexOfEnd = _myBag.tree.LastItemInRange(_rangeTester, out dummy);
                    return indexOfEnd - indexInSet;
                }
                else {
                    var indexInSet = _myBag.tree.FindIndex(item, true);
                    if(indexInSet < 0)
                        return -1;

                    var indexOfStart = _myBag.tree.FirstItemInRange(_rangeTester, out dummy);
        ...
        public class View : CollectionBase<T> {
            ...
            public override sealed IEnumerator<T> GetEnumerator() {
                if(_reversed)
                    return _myBag.tree.EnumerateRangeReversed(_rangeTester).GetEnumerator();

                return _myBag.tree.EnumerateRange(_rangeTester).GetEnumerator();
            }
            ...
            public override sealed void Clear() {
                if(_entireTree) {
                    ...
                    _myBag.tree.DeleteRange(_rangeTester);
                }
            }
            ...
            public override sealed bool Remove(T item) {
                return ItemInView(item) && _myBag.Remove(item);
            }
            ...
            public override sealed bool Contains(T item) {
                return ItemInView(item) && _myBag.Contains(item);
            }
            ...
            public int IndexOf(T item) {
                if(_entireTree) {
                    if(_reversed) {
                        int indexInSet = _myBag.tree.FindIndex(item, false);
                        if(indexInSet < 0)
                            return -1;

                        return _myBag.Count - 1 - indexInSet;
                    }
                    return _myBag.tree.FindIndex(item, true);
                }
            ...
            public int LastIndexOf(T item) {
                if(_entireTree) {
                    if(_reversed) {
                        int indexInSet = _myBag.tree.FindIndex(item, true);
                        if(indexInSet < 0)
                            return -1;
                    ...
                    return _myBag.tree.FindIndex(item, false);
                }
            ...
            public T GetFirst() {
                ...
                int found = _reversed
                                ? _myBag.tree.LastItemInRange(_rangeTester, out item)
                                : _myBag.tree.FirstItemInRange(_rangeTester, out item);
