...
namespace NSoft.NFramework.Collections.PowerCollections {
    ...
    public class OrderedDictionary<TKey, TValue> : DictionaryBase<TKey, TValue> {
        ...
        private readonly IComparer<KeyValuePair<TKey, TValue>> _pairComparer;
        ...
        private OrderedDictionary(IEnumerable<KeyValuePair<TKey, TValue>> keysAndValues, IComparer<TKey> keyComparer,
                                  IComparer<KeyValuePair<TKey, TValue>> pairComparer) {
        ...
        private OrderedDictionary(IComparer<TKey> keyComparer, IComparer<KeyValuePair<TKey, TValue>> pairComparer,
        ...
        public OrderedDictionary<TKey, TValue> Clone() {
            return new OrderedDictionary<TKey, TValue>(_keyComparer, _pairComparer, _tree.Clone());
        }
        ...
        public OrderedDictionary<TKey, TValue> CloneContents() {
            ...
            var newDict = new OrderedDictionary<TKey, TValue>(null, _keyComparer, _pairComparer);
            ...
            foreach(KeyValuePair<TKey, TValue> pair in _tree) {
                ...
                if(keyIsValueType)
                    ...
                    keyClone = Equals(pair.Key, null) ? default(TKey) : (TKey)pair.Key; //(TKey)(((ICloneable) pair.Key).Clone());
                }

                if(valueIsValueType)
                    valueClone = pair.Value;
                else {
                    valueClone = Equals(pair.Value, null) ? default(TValue) : (TValue)pair.Value;
                        //(TValue) (((ICloneable) pair.Value).Clone());
                }
        ...
        public override sealed bool Remove(TKey key) {
            ...
            return _tree.Delete(NewPair(key), true, out item);
        }
        ...
        public bool GetValueElseAdd(TKey key, ref TValue value) {
            ...
            var added = _tree.Insert(NewPair(key, value), DuplicatePolicy.DoNothing, out old);
        ...
        public override sealed void Add(TKey key, TValue value) {
            ...
            var added = _tree.Insert(pair, DuplicatePolicy.DoNothing, out dummy);
            if(!added)
        ...
        public void Replace(TKey key, TValue value) {
            ...
            var found = _tree.Find(pair, true, true, out dummy);
            if(!found)
        ...
        public void AddMany(IEnumerable<KeyValuePair<TKey, TValue>> keysAndValues) {
            keysAndValues.ShouldNotBeNull("keysAndValues");
        ...
        public int RemoveMany(IEnumerable<TKey> keyCollectionToRemove) {
            keyCollectionToRemove.ShouldNotBeNull("keyCollectionToRemove");
            return keyCollectionToRemove.Count(key => Remove(key));
        }
        ...
        public override sealed TValue this[TKey key] {
            get {
                ...
                var found = _tree.Find(NewPair(key), false, false, out pairFound);
            ...
            set {
                KeyValuePair<TKey, TValue> dummy;
                _tree.Insert(NewPair(key, value), DuplicatePolicy.ReplaceLast, out dummy);
            }
        }
        ...
        public class View : DictionaryBase<TKey, TValue> {
            ...
            internal View(OrderedDictionary<TKey, TValue> myDictionary, RedBlackTree<KeyValuePair<TKey, TValue>>.RangeTester rangeTester,
                          bool entireTree, bool reversed) {
            ...
            public override sealed int Count {
                get {
                    return _entireTree
                               ? _myDictionary.Count
                               : _myDictionary._tree.CountRange(_rangeTester);
                }
            }
            ...
            public override sealed bool ContainsKey(TKey key) {
                return KeyInView(key) && _myDictionary.ContainsKey(key);
            }
