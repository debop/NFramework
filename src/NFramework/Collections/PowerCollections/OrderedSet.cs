...
namespace NSoft.NFramework.Collections.PowerCollections {
    ...
    public class OrderedSet<T> : CollectionBase<T>, ICollection<T> {
        ...
        /// <exception cref="InvalidOperationException">T does not implement IComparable&lt;TKey&gt;.</exception>
        public OrderedSet(IEnumerable<T> collection) : this(collection, Comparers.DefaultComparer<T>()) {}
        ...
        public OrderedSet<T> Clone() {
            return new OrderedSet<T>(_comparer, _tree.Clone());
        }
        ...
        public OrderedSet<T> CloneContents() {
            ...
            var clone = new OrderedSet<T>(_comparer);

            // Clone each item, and add it to the new ordered set.
            foreach(var item in this) {
                T itemClone = itemIsValueType
                                  ? item
                                  : (Equals(item, null)
                                         ? default(T)
                                         : (T)item); //(T) (((ICloneable) item).Clone()));
        ...
        public bool IsSupersetOf(OrderedSet<T> otherSet) {
            ...
            return otherSet.All(item => Contains(item));
        }
        ...
        public OrderedSet<T> Intersection(OrderedSet<T> otherSet) {
            ...
            var result = new OrderedSet<T>(_comparer);
        ...
        private class ListView : ReadOnlyListBase<T> {
            ...
            public override T this[int index] {
                get {
                    if(_entireTree) {
                        return _reversed ? _mySet[_mySet.Count - 1 - index] : _mySet[index];
                    }
                    ...
                    return _reversed ? _mySet[lastIndex - index] : _mySet[firstIndex + index];
                }
            }

            public override int IndexOf(T item) {
                if(_entireTree) {
                    return _reversed ? _mySet.Count - 1 - _mySet.IndexOf(item) : _mySet.IndexOf(item);
                }
        ...
        public class View : CollectionBase<T>, ICollection<T> {
            ...
            public override sealed bool Remove(T item) {
                return ItemInView(item) && _mySet.Remove(item);
            }
            ...
            public override sealed bool Contains(T item) {
                return ItemInView(item) && _mySet.Contains(item);
            }
            ...
            public T GetFirst() {
                ...
                int found = _reversed
                                ? _mySet._tree.LastItemInRange(_rangeTester, out item)
                                : _mySet._tree.FirstItemInRange(_rangeTester, out item);
