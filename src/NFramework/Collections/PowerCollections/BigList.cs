...
namespace NSoft.NFramework.Collections.PowerCollections {
    ...
    public class BigList<T> : ListBase<T> {
        ...
        // Maximum number of elements in a leaf node -- small for debugging purposes.
#else
        ...
        private const int BALANCEFACTOR = 6;
                          // how far the root must be in depth from fully balanced to invoke the rebalance operation (min 3).
        ...
        public override sealed T this[int index] {
            get {
                ...
                while(curConcat != null) {
                    int leftCount = curConcat._left.Count;
                    if(index < leftCount)
                        current = curConcat._left;
                    else {
                        current = curConcat._right;
                        index -= leftCount;
                    }
            ...
            set {
                ...
                while(curConcat != null) {
                    ...
                    if(index < leftCount) {
                        current = curConcat._left;
                        if(current.Shared) {
                            curConcat._left = current.SetAt(index, value);
                            return;
                        }
                    }
                    else {
                        ...
                        index -= leftCount;
                        if(current.Shared) {
                            curConcat._right = current.SetAt(index, value);
                            return;
                        }
                    }
        ...
        public void AddRange(IEnumerable<T> collection) {
            ...
            if(root == null) {
                ...
                if((uint)Count + (uint)node._count > MAXITEMS)
                    throw new InvalidOperationException(Strings.CollectionTooLarge);
        ...
        public BigList<T> CloneContents() {
            ...
            return
                new BigList<T>(Algorithms.Convert(this,
            ...
            if(root != null && maxItems > 0) {
                ...
                if(start != 0) {
                    ...
                    while(currentConcat != null) {
                        ...
                        int leftCount = currentConcat._left.Count;
                        if(startIndex < leftCount) {
                            leftStack[stackPtr] = true;
                            current = currentConcat._left;
                        }
                        else {
                            leftStack[stackPtr] = false;
                            current = currentConcat._right;
                ...
                for(;;) {
                    ...
                    // Iterate the leaf.
                    var currentLeaf = (LeafNode)current;
        ...
        private static Node NodeFromEnumerable(IEnumerable<T> collection) {
            ...
            while((leaf = LeafFromEnumerator(enumerator)) != null) {
                if(node == null)
                    ...
                    if((uint)(node._count) + (uint)(leaf._count) > MAXITEMS)
                        throw new InvalidOperationException(Strings.CollectionTooLarge);
        ...
        private static LeafNode LeafFromEnumerator(IEnumerator<T> enumerator) {
            ...
            return (items != null) ? new LeafNode(i, items) : null;
        }
        ...
        private static Node NCopiesOfNode(int copies, Node node) {
            ...
            if((long)copies * (long)(node._count) > MAXITEMS)
                throw new InvalidOperationException(Strings.CollectionTooLarge);
        ...
        private void CheckBalance() {
            if(root != null)
                if(root.Depth > BALANCEFACTOR &&
                   !(root.Depth - BALANCEFACTOR <= MAXFIB && Count >= FIBONACCI[root.Depth - BALANCEFACTOR])) {
                    Rebalance();
                }
        }
        ...
        private static void AddNodeToRebalanceArray(Node[] rebalanceArray, Node node, bool shared) {
            if(node.Shared)
                shared = true;
            ...
            if(node.IsBalanced()) {
                ...
                AddNodeToRebalanceArray(rebalanceArray, n._left, shared);
                AddNodeToRebalanceArray(rebalanceArray, n._right, shared);
            }
        }
        ...
        private static void AddBalancedNodeToRebalanceArray(IList<Node> rebalanceArray, Node balancedNode) {
            Guard.Assert(balancedNode.IsBalanced(), "balancedNode.IsBalanced() 가 True이여야 합니다.");
            ...
            for(var i = 0; i < rebalanceArray.Count; ++i) {
        ...
        public int BinarySearch(T item, IComparer<T> comparer) {
            int index;
            var count = Algorithms.BinarySearch<T>(this, item, comparer, out index);
        ...
        private abstract class Node {
            ...
            protected volatile bool _shared;
            ...
            protected Node Prepend(Node node, bool nodeIsUnused) {
                return nodeIsUnused ? node.AppendInPlace(this, false) : node.Append(this, false);
            }
            ...
            public Node PrependInPlace(Node node, bool nodeIsUnused) {
                return nodeIsUnused ? node.AppendInPlace(this, !_shared) : node.Append(this, !_shared);
            }
        ...
        private sealed class LeafNode : Node {
            ...
            public LeafNode(int count, T[] newItems) {
                newItems.ShouldNotBeNull("newItems");
                Guard.Assert(count > 0 && count <= newItems.Length, @"count [{0}] 이 0보다 크고, newItems.Length[{1}] 이하여야 합니다.", count,
                             newItems.Length);
                newItems.Length.ShouldBeInRange(1, MAXLEAF + 1, "newItems.Length");
            ...
            public override Node SetAt(int index, T item) {
                ...
                newItems[index] = item;
                return new LeafNode(_count, newItems);
            }
            ...
            private bool MergeLeafInPlace(Node other) {
                Guard.Assert(_shared == false, "shared 가 false여야만 합니다.");
                ...
                if(otherLeaf != null && (newCount = otherLeaf.Count + _count) <= MAXLEAF) {
                    ...
                    if(newCount > items.Length) {
                        ...
                        Array.Copy(items, 0, newItems, 0, _count);
                        items = newItems;
                    }
                    Array.Copy(otherLeaf.items, 0, items, _count, otherLeaf._count);
                    _count = newCount;
                    return true;
                }
                return false;
            ...
            private Node MergeLeaf(Node other) {
                ...
                if(otherLeaf != null && (newCount = otherLeaf.Count + _count) <= MAXLEAF) {
                    ...
                    T[] newItems = new T[MAXLEAF];
                    Array.Copy(items, 0, newItems, 0, _count);
                    Array.Copy(otherLeaf.items, 0, newItems, _count, otherLeaf._count);
            ...
            public override Node PrependInPlace(T item) {
                ...
                if(_count < MAXLEAF) {
                    if(_count == items.Length) {
                        T[] newItems = new T[MAXLEAF];
                        Array.Copy(items, 0, newItems, 1, _count);
                        items = newItems;
                    }
                    else {
                        Array.Copy(items, 0, items, 1, _count);
                    }
            ...
            public override Node AppendInPlace(Node node, bool nodeIsUnused) {
                ...
                if(otherConcat != null && MergeLeafInPlace(otherConcat._left)) {
                    ...
                    return new ConcatNode(this, otherConcat._right);
                }
            ...
            public override Node Append(Node node, bool nodeIsUnused) {
                ...
                if(otherConcat != null && (result = MergeLeaf(otherConcat._left)) != null) {
                    if(!nodeIsUnused)
                        otherConcat._right.MarkShared();
                    return new ConcatNode(result, otherConcat._right);
                }
            ...
            public override Node InsertInPlace(int index, T item) {
                ...
                if(_count < MAXLEAF) {
                    if(_count == items.Length) {
                        ...
                        if(_count > index)
                            Array.Copy(items, index, newItems, index + 1, _count - index);
                        items = newItems;
                    }
                    else {
                        if(_count > index)
                ...
                T[] rightItems = new T[_count - index];
                Array.Copy(items, index, rightItems, 0, _count - index);
                Node rightNode = new LeafNode(_count - index, rightItems);
            ...
            public override Node InsertInPlace(int index, Node node, bool nodeIsUnused) {
                ...
                if(otherLeaf != null && (newCount = otherLeaf.Count + _count) <= MAXLEAF) {
                    ...
                    if(newCount > items.Length) {
                        ...
                        Array.Copy(items, index, newItems, index + otherLeaf.Count, _count - index);
                        items = newItems;
                    }
                    else {
                        Array.Copy(items, index, items, index + otherLeaf.Count, _count - index);
                        Array.Copy(otherLeaf.items, 0, items, index, otherLeaf._count);
                    }
                    _count = newCount;
                    return this;
                }
            ...
            public override Node Insert(int index, Node node, bool nodeIsUnused) {
                ...
                var rightItems = new T[_count - index];
                Array.Copy(items, index, rightItems, 0, _count - index);
                Node rightNode = new LeafNode(_count - index, rightItems);
            ...
            public override Node RemoveRangeInPlace(int first, int last) {
                ...
                Guard.Assert(first <= last, "first[{0}] <= last[{1}] 이어야 합니다.", first, last);
                last.ShouldBePositiveOrZero("last");

                if(first <= 0 && last >= _count - 1) {
                ...
                var newCount = first + (_count - last - 1); // number of items remaining.

                if(_count > last + 1)
                    Array.Copy(items, last + 1, items, first, _count - last - 1);

                for(var i = newCount; i < _count; ++i)
                    items[i] = default(T);
            ...
            public override Node RemoveRange(int first, int last) {
                ...
                if(last >= _count)
                    last = _count - 1;
                int newCount = first + (_count - last - 1); // number of items remaining.
                T[] newItems = new T[newCount];
                if(first > 0)
                    Array.Copy(items, 0, newItems, 0, first);
                if(_count > last + 1)
                    Array.Copy(items, last + 1, newItems, first, _count - last - 1);
                return new LeafNode(newCount, newItems);
            }
            ...
            public override void Validate() {
                ...
                Debug.Assert(_count <= MAXLEAF);
                Debug.Assert(items.Length <= MAXLEAF);
                Debug.Assert(_count <= items.Length);
            }
            ...
            public override void Print(string prefixNode, string prefixChildren) {
                Console.Write("{0}LEAF {1} count={2}/{3} ", prefixNode, _shared ? "S" : " ", _count, items.Length);
        ...
        private sealed class ConcatNode : Node {
            ...
            public ConcatNode(Node left, Node right) {
                ...
                if(left.Depth > right.Depth)
                    _depth = (short)(left.Depth + 1);
                else
                    _depth = (short)(right.Depth + 1);
            }
            ...
            private Node NewNodeInPlace(Node newLeft, Node newRight) {
                ...
                if(_left.Depth > _right.Depth)
                    _depth = (short)(_left.Depth + 1);
                else
                    _depth = (short)(_right.Depth + 1);
                return this;
            }
            ...
            public override T GetAt(int index) {
                ...
                if(index < leftCount)
                    return _left.GetAt(index);

                return _right.GetAt(index - leftCount);
            }
            ...
            public override Node SetAtInPlace(int index, T item) {
                ...
                if(index < leftCount) {
                    var newLeft = _left.SetAtInPlace(index, item);

                    if(newLeft != _left)
                        return NewNodeInPlace(newLeft, _right);

                    return this;
                }

                Node newRight = _right.SetAtInPlace(index - leftCount, item);

                if(newRight != _right)
                    return NewNodeInPlace(_left, newRight);
            ...
            public override Node SetAt(int index, T item) {
                ...
                if(index < leftCount) {
                    return NewNode(_left.SetAt(index, item), _right);
                }
                return NewNode(_left, _right.SetAt(index - leftCount, item));
            }
            ...
            public override Node AppendInPlace(Node node, bool nodeIsUnused) {
                ...
                if(_right.Count + node.Count <= MAXLEAF && _right is LeafNode && node is LeafNode)
                    return NewNodeInPlace(_left, _right.AppendInPlace(node, nodeIsUnused));
            ...
            public override Node Append(Node node, bool nodeIsUnused) {
                ...
                if(_right.Count + node.Count <= MAXLEAF && _right is LeafNode && node is LeafNode)
                    return NewNode(_left, _right.Append(node, nodeIsUnused));
            ...
            public override Node InsertInPlace(int index, T item) {
                ...
                var leftCount = _left.Count;
                if(index <= leftCount)
                    return NewNodeInPlace(_left.InsertInPlace(index, item), _right);
                return NewNodeInPlace(_left, _right.InsertInPlace(index - leftCount, item));
            }
            ...
            public override Node InsertInPlace(int index, Node node, bool nodeIsUnused) {
                ...
                var leftCount = _left.Count;
                if(index < leftCount)
                    return NewNodeInPlace(_left.InsertInPlace(index, node, nodeIsUnused), _right);
                return NewNodeInPlace(_left, _right.InsertInPlace(index - leftCount, node, nodeIsUnused));
            }
            ...
            public override Node Insert(int index, Node node, bool nodeIsUnused) {
                ...
                if(index < leftCount)
                    return NewNode(_left.Insert(index, node, nodeIsUnused), _right);
                return NewNode(_left, _right.Insert(index - leftCount, node, nodeIsUnused));
            }
            ...
            public override Node RemoveRangeInPlace(int first, int last) {
                ...
                var leftCount = _left.Count;
                Node newLeft = _left, newRight = _right;

                // Is part of the left being removed?
                if(first < leftCount)
                    newLeft = _left.RemoveRangeInPlace(first, last);
                // Is part of the right being remove?
                if(last >= leftCount)
                    newRight = _right.RemoveRangeInPlace(first - leftCount, last - leftCount);
            ...
            public override Node RemoveRange(int first, int last) {
                ...
                // Is part of the left being removed?
                if(first < leftCount)
                    newLeft = _left.RemoveRange(first, last);
                // Is part of the right being remove?
                if(last >= leftCount)
                    newRight = _right.RemoveRange(first - leftCount, last - leftCount);
            ...
            public override Node Subrange(int first, int last) {
                ...
                if(first < leftCount)
                    leftPart = _left.Subrange(first, last);
                // Is part of the right included?
                if(last >= leftCount)
                    rightPart = _right.Subrange(first - leftCount, last - leftCount);
            ...
            public override void Validate() {
                ...
                Debug.Assert(Count > 0);
                Debug.Assert(Math.Max(_left.Depth, _right.Depth) + 1 == Depth);
                Debug.Assert(_left.Count + _right.Count == Count);
                _left.Validate();
                _right.Validate();
            }
            ...
            /// <param name="prefixChildren">Prefixed to use in front of children of this node.</param>
            public override void Print(string prefixNode, string prefixChildren) {
                Console.WriteLine("{0}CONCAT {1} {2} count={3} depth={4}", prefixNode, _shared ? "S" : " ",
                                  IsBalanced() ? "B" : (IsAlmostBalanced() ? "A" : " "), _count, _depth);
                _left.Print(prefixChildren + "|-L-", prefixChildren + "|  ");
                _right.Print(prefixChildren + "|-R-", prefixChildren + "   ");
            }
#endif
            //DEBUG
        }
        ...
        private class BigListRange : ListBase<T> {
            private readonly BigList<T> _wrappedList;
            ...
            public override IEnumerator<T> GetEnumerator() {
                return _wrappedList.GetEnumerator(_start, _count);
            }
        }
    }
}