...
namespace NSoft.NFramework.Collections.PowerCollections {
    ...
    public class Set<T> : CollectionBase<T>, ICollection<T> {
        ...
        public Set<T> Clone() {
            return new Set<T>(_equalityComparer, _hash.Clone(null));
        }
        ...
        public Set<T> CloneContents() {
            ...
            var clone = new Set<T>(_equalityComparer);

            // Clone each item, and add it to the new ordered set.
            foreach(var item in this) {
                var itemClone = itemIsValueType
                                    ? item
                                    : (Equals(item, null)
                                           ? default(T)
        ...
        private void CheckConsistentComparison(Set<T> otherSet) {
            otherSet.ShouldNotBeNull("otherSet");
        ...
        public bool IsEqualTo(Set<T> otherSet) {
            ...
            return otherSet.Count == Count && otherSet.All(item => Contains(item));
        ...
        public void IntersectionWith(Set<T> otherSet) {
            ...
            var newHash = new Hash<T>(_equalityComparer);
        ...
        public Set<T> Intersection(Set<T> otherSet) {
            ...
            var result = new Set<T>(_equalityComparer);
            foreach(T item in smaller) {
                if(larger.Contains(item))
                    result.Add(item);
            }
        ...
        public void SymmetricDifferenceWith(Set<T> otherSet) {
            ...
            if(otherSet.Count > Count) {
                _hash.StopEnumerations();
                var newHash = otherSet._hash.Clone(null);
