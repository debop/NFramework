...
namespace NSoft.NFramework.Caching.Memcached.Web {
    ...
    /// </code>
    /// </example>
    public class MemcachedOutputCacheProvider : AbstractOutputCacheProvider {
        #region << logger >>
    ...
    //        var value = CacheRepository.Get(key);

    //        if(IsDebugEnabled)
    //            log.Debug("ASP.NET Page OutputCache를 로드했습니다. key=[{0}], value=[{1}]", key, value);
