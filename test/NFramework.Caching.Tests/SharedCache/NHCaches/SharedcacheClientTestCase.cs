...
namespace NSoft.NFramework.Caching.SharedCache.NHCaches {
    [TestFixture]
    public class SharedCacheClientTestCase {
        #region << logger >>
        ...
        public void Build_Without_Properties() {
            var cache = new SharedCacheClient("rcl");
            Assert.IsNotNull(cache);
        }
        ...
        public void Put_Null_Key() {
            Assert.Throws<ArgumentNullException>(() => new SharedCacheClient().Put(null, "value"));
        }

        [Test]
        public void Put_Null_Value() {
            Assert.Throws<ArgumentNullException>(() => new SharedCacheClient().Put("key", null));
        }
