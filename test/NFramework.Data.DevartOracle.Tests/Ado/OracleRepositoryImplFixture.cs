...
namespace NSoft.NFramework.Data.DevartOracle.Ado {
    [TestFixture]
    public class OracleRepositoryImplFixture : OracleFixtureBase {
        #region << logger >>
        ...
        public static IOracleRepository OracleRepository {
        ...
        public void Can_Create_Command_By_NamedQueryCommand_With_QueryKey(string queryKey) {
            if(OracleRepository.QueryProvider != null) {
                using(var cmd = OracleRepository.GetNamedQueryCommand(queryKey))
                using(var dataTable = OracleRepository.ExecuteDataTable(cmd)) {
                ...
                using(var dataTable = OracleRepository.ExecuteDataTable(queryKey)) {
                    Assert.IsNotNull(dataTable);
                    Assert.IsFalse(dataTable.HasErrors);
                    Assert.IsTrue(dataTable.Rows.Count > 0);
                }
        ...
        public void Can_Create_Command_By_NamedQueryCommand_With_Section_And_QueryName(string section, string queryName) {
            if(OracleRepository.QueryProvider != null) {
                using(var cmd = OracleRepository.GetNamedQueryCommand(section, queryName))
                using(var dataTable = OracleRepository.ExecuteDataTable(cmd)) {
                    Assert.IsNotNull(dataTable);
                    Assert.IsFalse(dataTable.HasErrors);
                    Assert.IsTrue(dataTable.Rows.Count > 0);
        ...
        public void ExecuteDataSet(int firstResult, int maxResults) {
            using(var cmd = OracleRepository.GetCommand(SelectEmp))
            using(var ds = OracleRepository.ExecuteDataSet(cmd, firstResult, maxResults)) {
            ...
            using(var cmd = OracleRepository.GetCommand(SelectDemoCustomers))
            using(var ds = OracleRepository.ExecuteDataSet(cmd, firstResult, maxResults)) {
                Assert.AreEqual(ds.Tables.Count, 1);
                Assert.IsFalse(ds.Tables[0].HasErrors);
                Assert.Greater(ds.Tables[0].Rows.Count, 0);
        ...
        public void ExecuteDataSetByQueryDirectly(int firstResult, int maxResults) {
            using(var ds = OracleRepository.ExecuteDataSet(SelectEmp, firstResult, maxResults)) {
                Assert.AreEqual(ds.Tables.Count, 1);
                Assert.IsFalse(ds.Tables[0].HasErrors);
                Assert.Greater(ds.Tables[0].Rows.Count, 0);
            }
            ...
            using(var ds = OracleRepository.ExecuteDataSet(SelectDemoCustomers, firstResult, maxResults)) {
                Assert.AreEqual(ds.Tables.Count, 1);
                Assert.IsFalse(ds.Tables[0].HasErrors);
                Assert.Greater(ds.Tables[0].Rows.Count, 0);
            }
        ...
        public void ExecuteDataSetInTransactionScope() {
            Action<int, int> @loadMethod =
                (firstResult, maxResults) => {
                    using(var ds = OracleRepository.ExecuteDataSet(SelectDemoCustomers,
                                                                   firstResult, maxResults)) {
                        Assert.AreEqual(ds.Tables.Count, 1);
                        Assert.IsFalse(ds.Tables[0].HasErrors);
                        Assert.Greater(ds.Tables[0].Rows.Count, 0);
        ...
        public void ExecuteDataTableByProcedure(int firstResult, int maxResults) {
            using(var table = OracleRepository.ExecuteDataTableByProcedure(SP_GET_EMPLOYEES_BY_DEPT,
                                                                           firstResult,
                                                                           maxResults,
                                                                           new AdoParameter("P_DEPTNO", 30,
                                                                                            DbType.Decimal))) {
        ...
        public void ExecutePagingDataTableTest(int pageIndex, int pageSize, double orderTotal) {
            ...
            using(new OperationTimer(title, false)) {
                using(var cmd = OracleRepository.GetCommand(selectSql))
                using(var pagingTable = OracleRepository.ExecutePagingDataTable(cmd, pageIndex, pageSize, parameters)) {
        ...
        public void ExecutePagingDataTableByQueryTest(int pageIndex, int pageSize, double orderTotal) {
            ...
            using(new OperationTimer(title)) {
                using(var pagingTable = OracleRepository.ExecutePagingDataTable(selectSql, pageIndex, pageSize, parameters)) {
                    Assert.AreEqual(pageIndex, pagingTable.PageIndex);
                    Assert.AreEqual(pageSize, pagingTable.PageSize);
        ...
        public void ExecutePagingDataTableBySelectSqlTest(int pageIndex, int pageSize, double orderTotal) {
            ...
            using(var pagingTable = OracleRepository.ExecutePagingDataTableBySqlString(selectSql, pageIndex, pageSize, parameters)) {
                Assert.AreEqual(pageIndex, pagingTable.PageIndex);
                Assert.AreEqual(pageSize, pagingTable.PageSize);
        ...
        public void ExecutePagingDataTableBySelectSqlTest(string selectSql, int pageIndex, int pageSize) {
            ...
            using(var pagingTable = OracleRepository.ExecutePagingDataTableBySqlString(selectSql, pageIndex, pageSize)) {
                Assert.AreEqual(pageIndex, pagingTable.PageIndex);
                Assert.AreEqual(pageSize, pagingTable.PageSize);
        ...
        #region << ExecuteInstance >>

        [Test]
        public void ExecuteReaderByQuery() {
            using(var reader = OracleRepository.ExecuteReaderBySqlString(SelectEmp)) {
        ...
        public void ExecuteReaderByProcedure() {
            using(var reader = OracleRepository.ExecuteReader(SelectDemoCustomers)) {
                reader.Read().Should().Be.True();
        ...
        public void ExecuteReaderWithPagingByNameMapper(int pageIndex, int pageSize) {
            using(var cmd = OracleRepository.GetCommand(SelectDemoStates)) {
                var jobs = OracleRepository.ExecuteInstance<State>(CapitalizeMapper, cmd, pageIndex, pageSize);
