...
namespace NSoft.NFramework.Collections.PowerCollections {
    ...
    public class HashTests {
        ...
        internal class DataComparer : IEqualityComparer<TestItem> {
        ...
        public void FindReplace() {
            ...
            bool b = FindReplaceKey("b", 202, 102);
            Assert.IsTrue(b);
        ...
        public void Enumerate() {
            ...
            var keys = new[] { "a2", "a5", "a7", "b", "c", "g", "h", "i", "k", "l", "m", "o", "p", "t" };
        ...
        public void InsertRandom() {
            for(var iter = 0; iter < ITERATIONS; ++iter) {
                hash = new Hash<TestItem>(new DataComparer());
                var a = CreateRandomArray(iter, LENGTH, LENGTH * 10, false);
                InsertArray(a);
#if DEBUG
                hash.Validate();
#endif
        ...
        public void FindRandom() {
            for(var iter = 0; iter < ITERATIONS; ++iter) {
                hash = new Hash<TestItem>(new DataComparer());
                var a = CreateRandomArray(iter + 1000, LENGTH, LENGTH * 10, false);
        ...
        public void DeleteRandom() {
            for(int iter = 0; iter < ITERATIONS / 10; ++iter) {
                ...
                for(var i = 0; i < LENGTH * 10; ++i) {
                    var v = rand.Next(LENGTH);
                    var key = StringFromInt(v);

                    if(a[v] && rand.Next(4) != 0) {
                        // Already in the hash. Make sure we can find it, then delete it.
                        var b = hash.Find(new TestItem(key), false, out itemFound);
        ...
        public void GrowShrink() {
            ...
            for(var i = 0; i < 600; ++i) {
                var b = hash1.Delete(r.NextDouble(), out dummy);
                Assert.IsTrue(b);
            }
        ...
        public void LoadFactor() {
            ...
            for(var i = 0; i < 600; ++i) {
                double dummy;
                var b = hash1.Insert(r.NextDouble(), true, out dummy);
                Assert.IsTrue(b);
            }
