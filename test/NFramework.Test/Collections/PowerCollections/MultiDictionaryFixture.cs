...
namespace NSoft.NFramework.Collections.PowerCollections
{
	...
	public class MultiDictionaryTests
	{
		...
		internal static void CheckMultiDictionaryContents<TKey, TValue>(MultiDictionary<TKey, TValue> dict, TKey[] keys, TValue[][] values, TKey nonKey, TValue nonValue, Func<TKey, TKey, bool> keyEquals, Func<TValue, TValue, bool> valueEquals)
		{
			int iKey, iValue;
		...
		public void Clone()
		{
			...
			dict2 = dict1.Clone(); //((MultiDictionary<string, string>)((ICloneable)dict1).Clone());
			...
			foreach(KeyValuePair<K, V> p1 in pairs1)
			{
				...
				for(int i = 0; i < pairs2.Length; ++i)
				{
					if(!found[i] && object.Equals(p1.Key, pairs2[i].Key) && object.Equals(p1.Value, pairs2[i].Value))
					{
						found[i] = true;
						f = true;
						// Assert.IsTrue(p1.Key == null || !object.Equals(p1.Key, pairs2[i].Key));
						// Assert.IsTrue(p1.Value == null || !object.Equals(p1.Value, pairs2[i].Value));
						break;
					}
				}
				Assert.IsTrue(f);
		...
		private class MyIntComparer : IEqualityComparer<OrderedDictionaryTests.MyInt>
		{
			public bool Equals(OrderedDictionaryTests.MyInt x, OrderedDictionaryTests.MyInt y)
			{
				if(x == null)
					return y == null;
				else if(y == null)
			...
			public int GetHashCode(OrderedDictionaryTests.MyInt obj)
			{
				if(obj == null)
					return 0x42834E;
				else
		...
		public void CloneContents()
		{
			...
			MultiDictionary<int, OrderedDictionaryTests.MyInt> dict1 = new MultiDictionary<int, OrderedDictionaryTests.MyInt>(true, EqualityComparer<int>.Default, comparer);
			...
			dict1.Add(7, null);
			dict1.Add(7, new OrderedDictionaryTests.MyInt(119));
			MultiDictionary<int, OrderedDictionaryTests.MyInt> dict2 = dict1.CloneContents();
			CompareClones(dict1, dict2);

			MultiDictionary<OrderedDictionaryTests.MyInt, int> dict3 = new MultiDictionary<OrderedDictionaryTests.MyInt, int>(false, comparer);
			...
			MultiDictionary<OrderedDictionaryTests.MyInt, int> dict4 = dict3.CloneContents();
			CompareClones(dict3, dict4);

			MultiDictionary<UtilFixture.CloneableStruct, UtilFixture.CloneableStruct> dict5 = new MultiDictionary<UtilFixture.CloneableStruct, UtilFixture.CloneableStruct>(true);
			...
			MultiDictionary<UtilFixture.CloneableStruct, UtilFixture.CloneableStruct> dict6 = dict5.CloneContents();

			Assert.IsTrue(dict5.Count == dict6.Count);

			IEnumerable<KeyValuePair<UtilFixture.CloneableStruct, UtilFixture.CloneableStruct>> e1 = dict5.KeyValuePairs;
			IEnumerable<KeyValuePair<UtilFixture.CloneableStruct, UtilFixture.CloneableStruct>> e2 = dict6.KeyValuePairs;
			KeyValuePair<UtilFixture.CloneableStruct, UtilFixture.CloneableStruct>[] pairs1 = Algorithms.ToArray<KeyValuePair<UtilFixture.CloneableStruct, UtilFixture.CloneableStruct>>(e1), pairs2 = Algorithms.ToArray<KeyValuePair<UtilFixture.CloneableStruct, UtilFixture.CloneableStruct>>(e2);
			bool[] found = new bool[pairs2.Length];

			// Check that the arrays are equal, but not reference equals (e.g., have been cloned).
			Assert.IsTrue(pairs1.Length == pairs2.Length);
			foreach(KeyValuePair<UtilFixture.CloneableStruct, UtilFixture.CloneableStruct> p1 in pairs1)
			{
				bool f = false;
				for(int i = 0; i < pairs2.Length; ++i)
				{
