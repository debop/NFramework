...
namespace NSoft.NFramework.Data.OdpNet.Ado {
    [TestFixture]
    public class OdpNetCommandAsyncFixture : OdpNetFixtureBase {
        #region << logger >>
        ...
        public OdpNetDatabase DefaultOracleDb {
        ...
        public void Can_Raise_Exception_When_Invalid_Query() {
            ...
            Assert.Throws<AggregateException>(() => {
                                                  using(var cmd = DefaultOracleDb.GetOracleCommand("SELECT * FRO TAB")) {
                                                      var table = OdpNetCommandAsync.ExecuteDataTableAsync(DefaultOracleDb, cmd).Result;
        ...
        public void CreateOracleConnectionAsynchronous() {
            ...
            using(var oraConn = OdpNetTool.CreateOracleConnection(DefaultOracleDb, ref newConnection)) {
