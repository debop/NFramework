...
namespace NSoft.NFramework.Data.MySql.Ado {
    [TestFixture]
    public class MySqlRepositoryImplFixture : MySqlAdoFixtureBase {
        #region << logger >>
        ...
        /// Repository
        /// </summary>
        public static IMySqlRepository Repository {
            get { return NorthwindMySqlRepository; }
        }
        ...
        public void ExecuteDataTableByCommandFrom_GetNamedQueryCommand(string queryKey) {
            ...
            using(var cmd = Repository.GetNamedQueryCommand(queryKey))
            using(var dataTable = Repository.ExecuteDataTable(cmd)) {
        ...
        public void ExecuteDataTableByQueryTest(string queryKey) {
            ...
            using(var dataTable = Repository.ExecuteDataTable(queryKey)) {
                VerifyDataTableHasData(dataTable);
            }
        }
        ...
        public void ExecuteDataTableByCommandFrom_GetNamedQueryCommand_Section_QueryName(string section, string queryName) {
            ...
            using(var cmd = Repository.GetNamedQueryCommand(section, queryName))
            using(var dataTable = Repository.ExecuteDataTable(cmd)) {
                VerifyDataTableHasData(dataTable);
            }
        }
        ...
        public void ExecuteDataTableByQuery_Section_QueryName(string section, string queryName) {
            ...
            // 내부에서 GetNamedQueryCommand를 생성합니다.
            using(var dataTable = Repository.ExecuteDataTable(section + "," + queryName)) {
                VerifyDataTableHasData(dataTable);
            }
        }
        ...
        public void ExecuteDataSetBySqlStringTest(int firstResult, int maxResults) {
            using(var ds = Repository.ExecuteDataSetBySqlString(SQL_ORDER_BY_ORDERDATE_AND_FREIGHT_AND_CUSTOMER_SELECT,
                                                                firstResult,
                                                                maxResults,
        ...
        public void ExecuteDataSetByQueryTest(int firstResult, int maxResults) {
            ...
            using(ds = Repository.ExecuteDataSet(SQL_ORDER_DETAIL_SELECT, firstResult, maxResults)) {
                ds.Tables.Count.Should().Be(1);
                var dataTable = ds.Tables[0];
                VerifyDataTableHasData(dataTable);
            }
        ...
        public void ExecuteDataSetBySqlStringWithPaging(int firstResult, int maxResults) {
            using(var ds = Repository.ExecuteDataSetBySqlString(SQL_ORDER_DETAIL_SELECT, firstResult, maxResults)) {
                ds.Tables.Count.Should().Be(1);
                var dataTable = ds.Tables[0];
                VerifyDataTableHasData(dataTable);
            }
        ...
        public virtual void ExecuteDataSetByProcedure() {
            using(var ds = Repository.ExecuteDataSetByProcedure(SP_CUSTOMER_ORDER_HISTORY, CustomerTestParameter)) {
                ds.Tables.Count.Should().Be(1);
                var dataTable = ds.Tables[0];
                VerifyDataTableHasData(dataTable);
            }
        ...
        public virtual void ExecuteDataSetByProcedureWithPaging(int firstResult, int maxResults) {
            using(
                var ds = Repository.ExecuteDataSetByProcedure(SP_CUSTOMER_ORDER_HISTORY, firstResult, maxResults, CustomerTestParameter)
                ) {
                ds.Tables.Count.Should().Be(1);
                var dataTable = ds.Tables[0];
                VerifyDataTableHasData(dataTable);
        ...
        public void ExecuteDataTable(int firstResult, int maxResults) {
            using(var cmd = Repository.GetCommand(SQL_ORDER_DETAIL_SELECT))
            using(var table = Repository.ExecuteDataTable(cmd, firstResult, maxResults)) {
        ...
        public void ExecuteDataTableByQuery(int firstResult, int maxResults) {
            using(var table = Repository.ExecuteDataTable(SQL_ORDER_DETAIL_SELECT, firstResult, maxResults)) {
                VerifyDataTableHasData(table);
            }
        }
        ...
        public void ExecuteDataTableBySqlString(int firstResult, int maxResults) {
            using(var table = Repository.ExecuteDataTableBySqlString(SQL_ORDER_SELECT, firstResult, maxResults)) {
                VerifyDataTableHasData(table);
            }
        }
        ...
        public virtual void ExecuteDataTableByProcedureTest(int firstResult, int maxResults) {
            using(var table = Repository.ExecuteDataTableByProcedure(SP_CUSTOMER_ORDER_HISTORY,
                                                                     firstResult,
                                                                     maxResults,
                                                                     CustomerTestParameter)) {
                VerifyDataTableHasData(table);
        ...
        public void ExecutePagingDataTableTest(int pageIndex, int pageSize, double discount) {
            ...
            using(
                ...
                using(var cmd = Repository.GetCommand(SQL_ORDER_DETAIL_SELECT + @" WHERE Discount > @Discount"))
                using(var pagingTable = Repository.ExecutePagingDataTable(cmd, pageIndex, pageSize, parameters)) {
        ...
        public void ExecutePagingDataTableBySelectSqlTest2(string selectSql, int pageIndex, int pageSize) {
            using(
                new OperationTimer(string.Format("ExecutePagingDataTableBySqlString(pageIndex=[{0}], pageSize=[{1}])", pageIndex,
                                                 pageSize)))
            using(var pagingTable = Repository.ExecutePagingDataTableBySqlString(selectSql, pageIndex, pageSize)) {
                Assert.AreEqual(pageIndex, pagingTable.PageIndex);
                Assert.AreEqual(pageSize, pagingTable.PageSize);
        ...
        public void ExecuteReaderByQuery() {
            using(var cmd = Repository.GetCommand(SQL_ORDER_DETAIL_SELECT))
            using(var reader = Repository.ExecuteReader(cmd)) {
        ...
        public virtual void ExecuteReaderByProcedure() {
            using(var cmd = Repository.GetCommand(SP_CUSTOMER_ORDER_HISTORY, CustomerTestParameter))
            using(var reader = Repository.ExecuteReader(cmd)) {
                reader.Count().Should().Be.GreaterThan(0);
            }
        }
        ...
        public void CountOfDataReader() {
            Repository.CountBySqlString("SELECT * FROM OrderDetails").Should().Be.GreaterThan(0);
            Repository.CountBySqlString("SELECT * FROM Customers").Should().Be.GreaterThan(0);
            Repository.CountBySqlString("SELECT * FROM Customers WHERE CustomerID IS NULL").Should().Be(0);

            // Oracle에서 없네?
            // UniRepository.CountByProcedure(SP_CUSTOMER_ORDER_HISTORY, CustomerTestParameter).Should().Be.GreaterThan(0);
        }
        ...
        public void ExistsByDataReader() {
            Repository.ExistsBySqlString("SELECT * FROM Customers").Should().Be.True();
            Repository.ExistsBySqlString("SELECT * FROM Customers WHERE CustomerID IS NULL").Should().Be.False();

            Repository.ExistsBySqlString("SELECT * FROM Customers WHERE CustomerID = @CustomerID",
                                         new AdoParameter("CustomerID", "DEBOP68"))
                .Should().Be.False();

            Repository.ExistsBySqlString("SELECT * FROM OrderDetails").Should().Be.True();

            // Oracle에서 Procedure를 만들지 않았습니다.
            // UniRepository.ExistsByProcedure(SP_CUSTOMER_ORDER_HISTORY, CustomerTestParameter).Should().Be.True();
        }
        ...
        public void ExecuteReaderToInstanceByNameMapping(int pageIndex, int pageSize) {
            using(var cmd = Repository.GetCommand(SQL_CUSTOMER_SELECT)) {
                var customers = Repository.ExecuteInstance<Customer>(TrimMapper, cmd, pageIndex, pageSize);
        ...
        public void ExecuteReaderWithPaging(int pageIndex, int pageSize) {
            using(var cmd = Repository.GetCommand(SQL_ORDER_DETAIL_SELECT)) {
                var orderDetails = Repository.ExecuteInstance<OrderDetail>(TrimMapper, cmd, pageIndex, pageSize);
        ...
        public virtual void ExecuteReaderToInstanceByConverter() {
            ...
            using(var cmd = Repository.GetCommand(SP_CUSTOMER_ORDER_HISTORY)) {
                var orderHistories = Repository.ExecuteInstance(@mapFunc, cmd, CustomerTestParameter);

                if(IsDebugEnabled)
                    log.Debug(orderHistories.CollectionToString());
            }
            using(var cmd = Repository.GetCommand(SP_CUSTOMER_ORDER_HISTORY)) {
                var orderHistories = Repository.ExecuteInstance(@mapFunc2, cmd, CustomerTestParameter);
        ...
        /// </summary>
        [Test]
        public virtual void ExecuteProcedure() {
            var outputs = Repository.ExecuteProcedure(SP_CUSTOMER_ORDER_HISTORY, CustomerTestParameter);
        ...
        public void FluentByNameMapper_Load() {
            ...
            using(var cmd = Repository.GetCommand(SP_CUSTOMER_ORDER_HISTORY)) {
                var orderHistories = Repository.ExecuteInstance<CustomerOrderHistory>(nameMapper, cmd, CustomerTestParameter);
